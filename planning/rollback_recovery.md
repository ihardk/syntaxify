...
Run 'meta_gen build --force' to regenerate.
```
...
To rollback: meta_gen rollback --to=2024-12-16_123000
```
...
```bash
$ meta_gen rollback --file=lib/components/app_button.dart
```
...
if [ $? -ne 0 ]; then
  ...
  echo "Run 'meta_gen build' to regenerate."
  exit 1
fi
```
...
# pubspec.yaml
dev_dependencies:
  meta_gen: 1.2.3  # Pin exact version
```
...
Run 'meta_gen update' to upgrade generator.
```
...
```bash
$ meta_gen config set backup.retention 10
```
